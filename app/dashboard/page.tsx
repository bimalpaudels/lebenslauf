...
export default function Dashboard() {
  ...
  const handleCreateNewCV = async (templateId: string) => {
    try {
      ...
      const template = tsxTemplates.find((t) => t.id === templateId);
      if (!template) return;

      const markdown =
        sampleMarkdown[templateId] || template.sampleMarkdown || "# New CV";
                ...
                return (
                  ...
                  >
                    <div className="flex-1 relative overflow-hidden">
                      <DashboardPreview
                        markdown={
                          sampleMarkdown[template.id] || template.description
                        }
                        className="h-full"
                        variant="template"
